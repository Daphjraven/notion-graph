...
function nodeIsRelevant(
  ...
  return links.some((link: GraphLink) => {
    const sourceId = getLinkNodeId(link.source);
    const targetId = getLinkNodeId(link.target);
...
export default function EmbedGraphPage({
  ...
  const activeNodeId = useMemo(() => {
    ...
    const match = graph.nodes.find((n: GraphNode) =>
  ...
  useEffect(() => {
    ...
    const nodes = graph.nodes.map((n: GraphNode) => ({ ...n }));
    const links = graph.links.map((l: GraphLink) => ({ ...l }));
    ...
    const node = g
      ...
      .attr("fill", (d: GraphNode) =>
        d.id === activeNodeId ? "#ffffff" : "#d9d9d9"
      )
      .attr("stroke", (d: GraphNode) =>
        d.id === activeNodeId ? "#7dd3fc" : "#f5f5f5"
      )
      .attr("stroke-width", (d: GraphNode) =>
        d.id === activeNodeId ? 2 : 0.8
      )
    ...
    const iconLabel = g
      ...
      .data(nodes.filter((d: GraphNode) => Boolean(d.emoji)))
      .join("text")
      .text((d: GraphNode) => d.emoji ?? "")
      .attr("font-size", 11)
    ...
    simulation.on("tick", () => {
      ...
      iconLabel
        ...
        .attr("y", (d: GraphNode) => (d.y ?? 0) - 10);

      label
        .attr("x", (d: GraphNode) => (d.x ?? 0) + 8)
        .attr("y", (d: GraphNode) => (d.y ?? 0) + 3);
    });
